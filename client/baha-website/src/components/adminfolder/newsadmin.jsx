...
export default function ManageNews({ setPage }) {
  ...
  const handleCreate = async (payload) => {
    const res = await axios.post(`${API_URL}/api/admin/news`, payload, { headers });
  ...
  const handleEdit = async (payload) => {
    const id = form._id;
    const res = await axios.patch(`${API_URL}/api/admin/news/${id}`, payload, { headers });
  ...
  // ── Delete ──
  const deleteNews = (id) => {
    axios.delete(`${API_URL}/api/admin/news/${id}`, { headers }).catch(() => {});
