...
import Navbar from "./Navbarhomepage.jsx";
...
function ProjectTeaser({ project, index, titleRef }) {
  ...
  return (
    <div
      ref={ref}
      onClick={() => navigate(`/Projectmodal/${project.slug}`)}
  ...
  <MobileMenu />
      ...
      <div className="flex flex-col lg:flex-row" style={{ minHeight: "100vh" }}>
        ...
        >
          ...
          {/* Passed project titles tracker */}
          {passedCount > 0 && (
            <div className="mt-6">
              {projects.slice(0, passedCount).map((p) => (
                <div
                  key={p._id || p.id}
                  className="flex justify-between border-t border-black py-1.5"
                >
        ...
        <div
          className="w-full lg:w-1/2 px-5 lg:px-6 pb-24"
      ...
      >
        <span
          ...
          style={{
            fontSize: "clamp(3rem, 10vw, 16rem)",
